...
import { Record, TagsRepo } from "../src/types";
...
describe("Record Contract", function () {
	async function deployRecordFixture() {
		...
		const tagRepo = (await TagRepo.deploy("testseedphrase")) as TagsRepo;

		const tagId = await tagRepo.tagMulti(
			[`${arwaeveBaseUrl}${fileTxs[0]}`, `${arwaeveBaseUrl}${fileTxs[1]}`],
			["", ""]
		);
		...
		const parsedTagIdLogs = tagIdLogs
			.map((x) => {
				return tagRepoAbiInterface.parseLog(x);
			})
			.filter((x) => {
				return x.name === "TagCreated";
			});

		const Record = await ethers.getContractFactory("Record");
		const parentRecord = await Record.deploy(
			[],
			"parentRecords",
			"IMG",
			BigInt(0)
	...
	describe("Checking Deployment", function () {
		...
		it("check if recordParentRecord is public and correct.", async function () {
			...
			expect(await record.recordParentRecords(0)).to.equal(
				parentRecord.address
			);
		});
	});
	...
	describe("Checking Tag Tracing Process", function () {
		describe("check if traceOneTag function works proper.", function () {
			it("check if function is not accessable when immutable", async function () {
				const { parsedTagIdLogs, tagRepo, parentRecord } = await loadFixture(
					deployRecordFixture
				);

				await expect(
					parentRecord.traceOneTag(
						tagRepo.address,
						parsedTagIdLogs[0].args.tagId
					)
				).to.be.revertedWith(
					`called function is only usable during building period.`
				);
			});
			it("check if function is accessable during building phase and data of Traced Tag is accessable.", async function () {
				const { parsedTagIdLogs, tagRepo, record } = await loadFixture(
					deployRecordFixture
				);

				const traceOneTagOrder = await record.traceOneTag(
					tagRepo.address,
					parsedTagIdLogs[0].args.tagId
				);
				const traceOneTagReciept = await traceOneTagOrder.wait();
				const traceOneTagLogs = traceOneTagReciept.logs;
				const parsedTraceOneTagLogs = traceOneTagLogs
					.map((x) => {
						return recordAbiInterface.parseLog(x);
					})
					.filter((x) => {
						return x.name === "TagTraced";
					});

				expect(parsedTagIdLogs[0].args.tagId).to.be.equal(
					parsedTraceOneTagLogs[0].args.tagId
				);
				expect(tagRepo.address).to.be.equal(
					parsedTraceOneTagLogs[0].args.tagRepoAddress
				);
			});
		});
		describe("check if traceMultiTags function works proper.", function () {
			it("check if function is not accessable when immutable", async function () {
				...
				await expect(
					parentRecord.traceMultiTags(tagRepo.address, [
						parsedTagIdLogs[0].args.tagId,
						parsedTagIdLogs[1].args.tagId,
					])
				).to.be.revertedWith(
					`called function is only usable during building period.`
				);
			...
			it("check if function is accessable during building phase and data of Traced Tags is accessable.", async function () {
				...
				const traceMultiTagsOrder = await record.traceMultiTags(
					tagRepo.address,
					[parsedTagIdLogs[0].args.tagId, parsedTagIdLogs[1].args.tagId]
				);
				const traceMultiTagsReciept = await traceMultiTagsOrder.wait();
				const traceMultiTagsLogs = traceMultiTagsReciept.logs;
				const parsedTraceMultiTagsLogs = traceMultiTagsLogs
					.map((x) => {
						return recordAbiInterface.parseLog(x);
					})
					.filter((x) => {
				...
				while (i < parsedTraceMultiTagsLogs.length) {
					expect(parsedTagIdLogs[i].args.tagId).to.be.equal(
						parsedTraceMultiTagsLogs[i].args.tagId
					);
					expect(tagRepo.address).to.be.equal(
						parsedTraceMultiTagsLogs[i].args.tagRepoAddress
					);
					i++;
				}
			});
	...
	describe("Checking Tag Signing Process", function () {
		describe("check if sign function works proper.", function () {
			it("check if function is not accessable during building phase.", async function () {
				...
				await expect(record.sign()).to.be.revertedWith(
					`called function is only available when contract is immutable.`
				);
			});
			it("check if function is accessable when immutable", async function () {
				...
				await expect(parentRecord.sign())
					.to.emit(parentRecord, "RecordSigned")
		...
		describe("check if hasSigned function works proper.", function () {
			it("check if function is not accessable during building phase.", async function () {
				...
				await expect(record.hasSigned(owner.address)).to.be.revertedWith(
					`called function is only available when contract is immutable.`
				);
			});
	...
	describe("Checking Disabled Function", function () {
		it("check if approve is disabled.", async function () {
			...
			await expect(
				record.approve(otherAccount.address, BigInt(0))
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for Records."
			);
		});
		it("check if getApproved is disabled.", async function () {
			const { record } = await loadFixture(deployRecordFixture);

			await expect(record.getApproved(BigInt(0))).to.be.revertedWith(
				"The invoked function of ERC721 is disable for Records."
			);
		});
		it("check if setApprovedForAll is disabled.", async function () {
			...
			await expect(
				record.setApprovalForAll(owner.address, true)
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for Records."
			);
		});
		it("check if isApprivedForAll is disabled.", async function () {
			const { record, owner, otherAccount } = await loadFixture(
				deployRecordFixture
			);

			await expect(
				record.isApprovedForAll(owner.address, otherAccount.address)
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for Records."
			);
		});
		it("check if transferFrom is disabled.", async function () {
			...
			await expect(
				record.transferFrom(owner.address, otherAccount.address, BigInt(0))
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for Records."
			);
		});
		it("check if safeTransferFrom_2 is disabled.", async function () {
			...
			await expect(
				record["safeTransferFrom(address,address,uint256)"](
					owner.address,
		...
		it("check if safeTransferFrom_3 is disabled.", async function () {
			...
			await expect(
				record["safeTransferFrom(address,address,uint256,bytes)"](
					owner.address,
					otherAccount.address,
					BigInt(0),
					"0x00"
