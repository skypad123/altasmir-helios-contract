...
import { RecordInterface } from "../src/types/contracts/Record.sol/Record";
import { TagsRepoInterface } from "../src/types/contracts/TagRepo.sol/TagsRepo";
import { TagsRepo } from "../src/types";
...
describe("TagRepo Contract", function () {
	...
	describe("Checking Tagging Process", function () {
		it("check if tagOne works.", async function () {
			...
			await expect(tagRepo.tagOne(`${arwaeveBaseUrl}${fileTxs[0]}`, ""))
				.to.emit(tagRepo, "TagCreated")
				.withArgs(anyValue, `${arwaeveBaseUrl}${fileTxs[0]}`, ["", ""]);
		});
		it("check if tagMulti works.", async function () {
			const uriArr = fileTxs.map((x) => {
				return `${arwaeveBaseUrl}${x}`;
			});
			const { tagRepo } = await loadFixture(deployTagRepoFixture);

			let count = 0;
			while (count < uriArr.length) {
				await expect(tagRepo.tagMulti(uriArr, ["", ""]))
					.to.emit(tagRepo, "TagCreated")
					.withArgs(anyValue, `${arwaeveBaseUrl}${fileTxs[count]}`, ["", ""]);
				count++;
			}
		});
	});
	describe("Checking Disabled Function", function () {
		it("check if approve is disabled.", async function () {
			...
			await expect(
				tagRepo.approve(otherAccount.address, BigInt(0))
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for tagRepos."
			);
		});
		it("check if getApproved is disabled.", async function () {
			const { tagRepo } = await loadFixture(deployTagRepoFixture);

			await expect(tagRepo.getApproved(BigInt(0))).to.be.revertedWith(
				"The invoked function of ERC721 is disable for tagRepos."
			);
		});
		it("check if setApprovedForAll is disabled.", async function () {
			...
			await expect(
				tagRepo.setApprovalForAll(owner.address, true)
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for tagRepos."
			);
		});
		it("check if isApprivedForAll is disabled.", async function () {
			const { tagRepo, owner, otherAccount } = await loadFixture(
				deployTagRepoFixture
			);

			await expect(
				tagRepo.isApprovedForAll(owner.address, otherAccount.address)
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for tagRepos."
			);
		});
		it("check if transferFrom is disabled.", async function () {
			...
			await expect(
				tagRepo.transferFrom(owner.address, otherAccount.address, BigInt(0))
			).to.be.revertedWith(
				"The invoked function of ERC721 is disable for tagRepos."
			);
		});
		it("check if safeTransferFrom_2 is disabled.", async function () {
			...
			await expect(
				tagRepo["safeTransferFrom(address,address,uint256)"](
					owner.address,
		...
		it("check if safeTransferFrom_3 is disabled.", async function () {
			...
			await expect(
				tagRepo["safeTransferFrom(address,address,uint256,bytes)"](
					owner.address,
					otherAccount.address,
					BigInt(0),
					"0x00"
